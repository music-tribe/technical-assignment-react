...
import { getOasis, getOasisReleases } from '../redux/actions/oasis'
...
class OasisPage extends React.Component {
  constructor (props) {
    super(props)
  ...
  render () {
    ...
    return (
      <div onClick={() => this.setState({ moreInfo: null })}>
        ...
        <h3>Releases:</h3>
        <Releases>
          {!releases.success && <button onClick={() => this.props.dispatch(getOasisReleases(profile.data.releases_url))}>Get Releases</button>}
          {releases.success && <ul>
...
export default connect(mapStateToProps)(OasisPage)
